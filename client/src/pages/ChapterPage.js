...
function ChapterPage() {
  ...
  useEffect(() => {
    if (viewInfor?.cooldown > 0) {
      setTimeout(() => setTimeoutAccepted(true), viewInfor?.cooldown * 1000);
    }
  }, [viewInfor]);
  ...
  }, [readAccepted, timeoutAccepted, dispatch, comicSlug, chapter, viewInfor]);
  ...
  return (
    <div className={classes.container}>
      ...
      <div className={classes.navbar_bottom}>
        ...
        <button className={`${classes.active} bg-gray-200`} onClick={openModal}>
          Chương {chapter}
        </button>
        ...
        <ul className={classes.model_content_list}>
          {chapterList?.map((item, index) => (
            <li
              key={index}
              className={
                item === parseInt(chapter) ? classes.active : undefined
              }
            >
              <Link to={`/chapter/${comicSlug}/${item}`} onClick={closeModal}>
                Chương {item}
              </Link>
            </li>
          ))}
